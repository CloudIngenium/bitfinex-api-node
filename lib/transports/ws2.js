...
class WSv2 extends EventEmitter {
  ...
  async close (code, reason) {
    ...
    // Clear pending order operations
    ...
    // Clear packet watchdog to prevent reconnect after close
  ...
  async _onWSClose () {
    ...
    // Save channel map BEFORE clearing so reconnect can resubscribe
    if (shouldReconnect) {
  ...
  _updateManagedOB (symbol, data, raw, rawMsg) {
    ...
    let rawLossless
    try {
      rawLossless = LosslessJSON.parse(rawMsg, (key, value) => {
  ...
  _propagateMessageToListeners (msg, chan, transform = this._transform) {
    // Snapshot listener groups to prevent issues if callbacks modify listeners
  ...
  /**
   * Returns a promise that resolves on event or rejects after timeout.
   * Used for order operations where hanging promises are dangerous.
   *
   * @param {string} key - key for the promise
   * @param {number} [timeoutMs] - timeout in ms, default 30000
   * @returns {Promise} p - resolves on event, rejects on timeout
   * @private
   */
